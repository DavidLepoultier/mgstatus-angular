...
export class DetailOfferComponent implements OnInit {
  ...
  getOffer(id: any) {
    this.offer.getOffer(id).subscribe(
      data => {
        this.config = data.offer;
        this.disabledLifetime = this.config.lifetime;
        this.disabledSilab = this.config.silab;
        this.createForms();
      }
    )
  }
  ...
  updateConfig(registerOfferForm: any) {
    ...
    this.offer.updateOfferConfig(this.route.snapshot.params['id'], registerOfferForm).subscribe(
  ...
  deleteOffer(){
    this.offer.deleteOffer(this.route.snapshot.params['id']).subscribe(
      data => this.handlerSuccess(data),
      error => this.handlerError(error)
    );
  }
